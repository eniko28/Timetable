    ...
    .then((existingData) => {
      if (!existingData) {
        ...
        return db
          ...
          .then(() => {})
          .catch((error) => {
            console.error(`Error inserting data: ${JSON.stringify(data)}`, error);
          });
      }
      return existingData;
    });
...
function createPropertyIfNotExists(classObj, propertyName, propertyType, linkedClassName = null) {
  return classObj.property
    ...
    .then((existingProperty) => {
      if (existingProperty === null) {
        ...
        return classObj.property.create(propertyConfig).then((createdProperty) => createdProperty);
      }
...
function createDataIfNotExistsClassrooms(db, className, data, keyName, keyValue) {
  return db
    .select()
    .from(className)
    .where({ [keyName]: keyValue })
  ...
  return db.class
    ...
    .then(() => {
      ...
      const classroomPromises = classroomData.map((classroom) =>
        createDataIfNotExistsClassrooms(db, className, classroom, 'classroomId', classroom.classroomId),
      );
  ...
  return db.class
    ...
    .then(() => {
      ...
      const groupPromises = groupData.map((group) =>
        createDataIfNotExistsGroups(db, className, group, 'id', group.id, ['gradeLevel']),
      );
  ...
  return db.class
    ...
    .then(() => {
      const userData = {
        userId: 'admin01',
        name: 'Admin',
        password: '$2b$10$PWTYAlbeqyHwbbMxgrGsB.kgk.IeukIce0dcAMkRjfY861u8iHLRS',
        type: 'Admin',
      };

      return createDataIfNotExists(db, className, userData, 'userId', 'admin01');
    })
    .catch((error) => {
