...
export const getGroupDetails = async (req, res) => {
  try {
    ...
    const groups = await groupDB.getGroupsByNameAndGradeLevel(db, name, gradeLevel);

    await Promise.all(
      groups.map(async (group) => {
        const teachings = await timetableDB.selectTimetableByGroupId(db, group.id);

        group.teachings = await Promise.all(
          teachings.map(async (teaching) => {
            const teacher = await teacherDB.getTeacherNameById(db, teaching.teacherId);
