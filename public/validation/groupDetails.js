document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateSubjectInput(subjects, timetableSubjects, selectedGroup) {
    ...
    const filteredSubjectsByGroup = subjects.filter(
      (subject) => subject.groupId === selectedGroup
    );
  ...
  listItems.forEach((item) => {
    item.addEventListener("click", () => {
      ...
      const previousTeacherWishlistCells =
        ...
        if (xhr.readyState === 4 && xhr.status === 200) {
          ...
          if (response.teachers.length > 0) {
            ...
            teacherInput.addEventListener("change", () => {
              ...
              const filteredSubjects = response.subjects.filter(
                (subject) => subject.teacherId === selectedTeacher
              );
              const timetableSubjects = response.timetable.map(
                (slot) => slot.subjectId
              );
              ...
              const teacherWishlists = response.wishlists.filter(
                (wishlist) => wishlist.teacherId === teacherId
              );
  ...
  timetableCells.forEach((cell) => {
    cell.addEventListener("click", () => {
      ...
      const startTime = cellId[1]
        ...
        .padStart(4, "0")
        .replace(/(..)(..)/, "$1$2");
      const endTime = `${(parseInt(startTime.split(":")[0], 10) + 2)
        .toString()
        .padStart(2, "0")}:00`;
      lastClickedCellData = {
        selectedDay,
